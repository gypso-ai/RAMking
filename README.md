...
`realloc`, `free`) and tracks every live allocation in a compile-time-sized
static record table.  It detects **double-free**, **unregistered-pointer free**,
and provides rich diagnostic APIs — all without calling `malloc` for its own
bookkeeping.
